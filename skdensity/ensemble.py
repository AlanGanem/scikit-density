...
__all__ = ['JointEstimator', 'JointEntropyEstimator', 'JointKernelTreeEstimator', 'QuantileCalibrator', 'identity_func',
           'EntropyEstimator', 'IDENTITY_TRANSFORMER', 'expected_likelihood', 'inverese_log_node_var', 'datapoint_pdf',
           'datapoint_gaussian_likelihood', 'AVALIBLE_NODE_AGG_FUNC', 'AVALIBLE_DATAPOINT_WEIGHT_FUNC',
           'TreeEstimatorMixin', 'KernelTreeEstimator', 'KernelTreeEntropyEstimator', 'BaggingDensityEstimator',
...
class JointEstimator(MultiOutputRegressor):
    ...
    The results are pseudo joint since it samples independently from the marginal
    '''
...
class JointEntropyEstimator(MultiOutputRegressor):
    ...
    The results are pseudo joint since it samples independently from the marginal
    '''
    #create custom chianed classifier, as a stacked estimator, one for each target
    def apply(self,X):
        result = [estim.apply(X) for estim in self.estimators_]
        return np.concatenate(result, axis = -1)
...
class QuantileCalibrator(BaseEstimator):
    ...
    def sample(self, X, sample_size = 100, weight_func = None,
               alpha = None, replace = True, noise_factor = 1e-3, **sampling_kws):
        '''
...
class EntropyEstimator(BaseEstimator, DelegateEstimatorMixIn):
    ...
    '''
    def __init__(self,estimator, resolution = 100, alpha = 1, calibrated_classifier = None, calibration_cv = 4,rv_bins_kws = {} ,**q_transformer_kws):
        assert hasattr(estimator, 'predict_proba'), 'estimator should implement `predict_proba` method'
        self.estimator = estimator
        self.q_transformer = QuantileTransformer(n_quantiles = resolution, **q_transformer_kws) #<-- pass object as argument??
        self.alpha = alpha
    ...
    def _preprocess_y_fit(self, y):
        ...
        if len(y.shape) == 1:
            ...
            # assert 1d
            assert y.shape[-1] == 1, 'y should be 1d. For joint estimation use KernelTreeEntropyEstimator'
    ...
    def custom_predict(self, X, agg_func = np.mean, sample_size = 100, weight_func = None, alpha = None, replace = True, noise_factor = 1e-3):
        '''
    ...
    def density(self, X, dist = 'kde', sample_size = 100, weight_func = None, alpha = None, replace = True, noise_factor = 1e-3, **dist_kws):
        '''
...
class KernelTreeEstimator(BaseEstimator, DelegateEstimatorMixIn ,TreeEstimatorMixin):
    ...
    def density(self, X, dist = 'kde', sample_size = 10, weight_func = None, n_neighbors = None,
               lower_bound = None, alpha = None, beta = None, gamma = None, noise_factor = 1e-3, **dist_kwargs):
...
class KernelTreeEntropyEstimator(KernelTreeEstimator):
    ...
    def __init__(self, estimator, entropy_estimator_sampler = None, resolution = 100, alpha = 1, beta = 1, gamma = 1, node_rank_func = None,
                 node_data_rank_func = None,n_neighbors = 30, lower_bound = 0.0, **q_transformer_kws):
        ...
        self.resolution = resolution
        self.q_transformer = QuantileTransformer(n_quantiles = self.resolution, **q_transformer_kws)
        self._q_minmax_scaler = MinMaxScaler()
